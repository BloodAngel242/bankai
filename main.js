...
const purge2Command = require('./commands/purge2');
// Global settings
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        const adminCommands = ['.mute', '.unmute', '.ban', '.unban', '.promote', '.demote', '.kick', '.tagall', '.tagnotadmin', '.hidetag', '.antilink', '.antitag', '.setgdesc', '.setgname', '.setgpp', '.purge', '.purge2'];
        const isAdminCommand = adminCommands.some(cmd => userMessage.startsWith(cmd));

        // List of owner commands
        const ownerCommands = ['.mode', '.autostatus', '.antidelete', '.cleartmp', '.setpp', '.clearsession', '.areact', '.autoreact', '.autotyping', '.autoread', '.pmblocker', '.purge', '.purge2'];
        ...
        if (isGroup && isAdminCommand) {
            ...
            if (!isBotAdmin) {
                await sock.sendMessage(chatId, { text: 'Veuillez nommé le bot administrateur pour executer cette commande.', ...channelInfo }, { quoted: message });
                return;
            }
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.unban'):
                ...
                break;
            case userMessage === '.purge':
    await purgeCommand(sock, chatId, senderId, message);
    break;
            case userMessage === '.purge2':
    await purge2Command(sock, chatId, senderId, message);
    break;
            case userMessage === '.help' || userMessage === '.menu' || userMessage === '.kurama' || userMessage === '.list':
